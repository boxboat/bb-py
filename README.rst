...
``bb-py`` is a collection of cloud utilities to simplify deployment in
the cloud.
...
-  Dynamically create EC2 Ansible inventory based on generic inventory
   definition file
...
-  Note ``vars`` and ``children`` are optional fields in the
   ``inventory`` dictionary.
-  You can define ``asg_groups`` or ``ec2_groups`` or a mixture as shown
   in the example
...
-  You can preview this output using ``--list`` as an argument to
   ``bb-ec2-inventory``
...
.. _sample-output-passed-to-ansible-1:
